    ...
    <div className="space-y-6">
      ...
      {clients.length === 0 ? (
        ...
        <ul className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {clients.map((c) => (
            <li key={c.id}>
              <div className="card flex h-full flex-col gap-3 p-5 transition hover:border-brand-500/50 hover:shadow-card">
                <Link href={`/admin/${c.id}`} className="group flex flex-1 flex-col gap-3">
                ...
                <div className="mt-auto flex items-center justify-between gap-3 text-xs text-ink-500">
                  <span>
                    {c._count.leads} lead{c._count.leads === 1 ? "" : "s"}
                  </span>
                  <div className="flex items-center gap-3">
                    <Link href={`/admin/${c.id}`} className="text-brand-600 hover:underline">
                      Manage
                    </Link>
